...
2. evidence는 25자 이내, '위치 + 상태' 구조의 구어체 1문장으로 쓴다(예: '코 옆 모공이 도드라져요'). 만연체와 '관찰됨/판정됨' 같은 보고서체 종결 금지. 반드시 위치를 명시하고("코 옆", "왼쪽 볼" 등), 근거를 댈 수 없으면 점수를 만들지 마라.
3. 모든 점수 지표(모공·피부결·트러블·유수분·색 지표)는 관찰량의 선형 반영이 아니라 5점 단위 구간(심각도 밴드)으로 산정한다. 트러블은 활성 병변(융기·염증)과 흔적(자국)을 구분해 산정하되, 판별이 경계선인 병변은 개수에서 제외한다.
4. oil_moisture.score는 유분·수분 밸런스의 '품질' 점수다. 균형 잡힌 상태 = 높은 점수(70~100), 유분 과다 또는 수분 부족 등 균형 이탈이 관찰될수록 감점한다. 유분이 많다는 관찰은 반드시 낮은 점수로 이어져야 한다. evidence에 '과다/부족/균형' 중 판정을 명시한다.
...
6. 색 지표(redness, tone_evenness, pigment)는 lighting이 "good"일 때만 점수화한다.
...
10. basis는 사용자에게 그대로 표시된다. 영어 지표명·변수명·'held' 등 내부 용어 금지, 한국어 지표명(모공·피부결·트러블·유수분)만 사용, 2문장 이내.
